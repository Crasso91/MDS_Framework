...
function SPAWN:InitHeading( HeadingMin, HeadingMax )
  if self.SpawnCommanded then
    error("Spawn already planned or done. You cannot call init function after spawn")
  end
...
--Crasso - InitSpawnConcept with flag check on initheading (just for test)

--spawn method enumerators
SpawnMethod = {
  Scheduled = "Scheduled",
  Instant = "Instant"
}

--defining spawn functions to select thos without the use of 'if then elseif else end'
SPAWN.CaseSpawnMethods = {
  [SpawnMethod.Instant] = function(self) 
    self:F( { self.SpawnTemplatePrefix, self.SpawnIndex, self.AliveUnits } )

    if self.SpawnInitAirbase then
      self.SpawnedGroup = self:SpawnAtAirbase(self.SpawnInitAirbase, self.SpawnInitTakeoff, nil, self.SpawnInitTerminalType)
    else
      self.SpawnedGroup = self:SpawnWithIndex( self.SpawnIndex + 1 )
    end
    return self
  end,
  [SpawnMethod.Scheduled] = function(self, SpawnTime, SpawnTimeVariation)
    self:F( { SpawnTime, SpawnTimeVariation } )
...
-- 
function SPAWN:InitSpawnMethod(_spawnMethod, _args)
   self.SpawnMethod = _spawnMethod
   self.SpawnMethodArgs = _args or {}
  return self
end

function SPAWN:SpawnFromInit()
  self.SpawnCommanded = true
  if self.SpawnMethod == nil then
    error("No spawning method selected")
  end
  local args = nil
  if UtilitiesService:Lenght(self.SpawnMethodArgs) > 0 then
    SPAWN.CaseSpawnMethods[self.SpawnMethod](self, unpack(self.SpawnMethodArgs))
  else
    SPAWN.CaseSpawnMethods[self.SpawnMethod](self)
  end
  return self
end

function SPAWN:GetGroup()
